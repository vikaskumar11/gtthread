...
extern int uthread_create(uthread_t *u_tid, int (*u_func)(void *), void *u_arg, unsigned int u_weight);
...
extern void uthread_schedule(uthread_struct_t * (*kthread_best_sched_uthread)(kthread_runqueue_t *))
{
     ...
     TAILQ_FOREACH(p_uthread, &(kthread_runq->runqueues[0].tq_head), uthread_runq)
     {
	  KLOG("inside activeQ cpuid[%d %d] uid[%d]\n",  k_ctx->cpuid, k_ctx->cpu_apic_id, p_uthread->uthread_tid);
     }
	
     TAILQ_FOREACH(p_uthread, &(kthread_runq->runqueues[1].tq_head), uthread_runq)
     {
	  KLOG("inside expireQ cpuid[%d %d] uid[%d]\n",  k_ctx->cpuid, k_ctx->cpu_apic_id, p_uthread->uthread_tid);
     }
#endif
     ...
     if((u_obj = kthread_runq->cur_uthread))
     {
	  ...
	  if(u_obj->uthread_state & (UTHREAD_DONE | UTHREAD_CANCELLED))
	  {
		...
		* cleanup/exit of uthread (core dump) */
	       kthread_runq->total_uthread_weight -= u_obj->uthread_weight;
	  ...
	  else
	  {
	       ...
	       if( 0 != gettimeofday(&t_now, 0))
	       {
		    perror("gettimeofday");
		    exit(1);
	       }
	       KLOG("uthread id[%d] tnow[%ld %ld] t_earl[%ld %ld]\n", u_obj->uthread_tid, t_now.tv_sec, t_now.tv_usec,u_obj->uthread_scheduled_time.tv_sec, u_obj->uthread_scheduled_time.tv_usec); 
	       timersub(&t_now, &u_obj->uthread_scheduled_time, &t_diff);
	       printf("time=[%ld %ld] uthread_credit=[%d]\n", t_diff.tv_sec, t_diff.tv_usec, u_obj->uthread_credit);
	       u_obj->uthread_credit -= (MICRO_SEC(t_diff.tv_sec) + t_diff.tv_usec); 
     ...
     if(!(u_obj = kthread_best_sched_uthread(kthread_runq)))
     {
	  ...
	  if(ksched_shared_info.kthread_cur_uthreads)
	  {
	       ksched_load_balance(); 
	  }	  
	  gt_spin_unlock(&ksched_info->ksched_lock);
     ...
     u_obj->uthread_schedule_count ++;

     /*save scheduled time*/
     timerclear(&u_obj->uthread_scheduled_time);
     if( 0 != gettimeofday(&u_obj->uthread_scheduled_time, 0))
     {
	  perror("gettimeofday");
	  exit(1);
     }
     KLOG("uthread id[%d] schedule on[%lu %lu]\n", u_obj->uthread_tid, u_obj->uthread_scheduled_time.tv_sec ,u_obj->uthread_scheduled_time.tv_usec);
...
extern int uthread_create(uthread_t *u_tid, int (*u_func)(void *), void *u_arg, unsigned int u_weight)
{
     ...
     u_new->uthread_credit = DEFAULT_UTHREAD_CREDIT;
     ...
     kthread_runq->total_uthread_weight +=  u_new->uthread_weight;
     PRN("kthread_runq=[%d][%d]\n", kthread_runq->runqueues[0].uthread_tot,kthread_runq->runqueues[1].uthread_tot );
