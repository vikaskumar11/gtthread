...
typedef struct uthread_struct
{
     ...
     unsigned int uthread_weight; /*assigned by user*/
     int uthread_credit; /*assigned and managed by credit scheduler*/
     struct timeval uthread_scheduled_time; /*time at which uthread is scheduled by credit scheduler*/
     int uthread_schedule_count; /*count the number of times uthread is scheduled by credit scheduler*/
...
//#define uthread_schedule(X)			uthread_schedule_t(X)
//printf("[%s][%s][%d]\n",__FILE__,__FUNCTION__,__LINE__);	\
//uthread_schedule_t(X);	
