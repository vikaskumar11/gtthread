...
void __rem_from_runqueue(runqueue_t *runq, uthread_struct_t *u_elm);
...
{
  uthread_head_t *uhead = &(runq->tq_head);
  TAILQ_INIT(uhead);
#if 0	
	int i, j;
...
extern uthread_struct_t *sched_find_best_uthread(kthread_runqueue_t *kthread_runq)
{
	...
	if( kthread_runq->active_runq->uthread_tot)
	  runq = kthread_runq->active_runq;
	else if ( kthread_runq->expires_runq->uthread_tot)
