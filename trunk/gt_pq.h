...
#define REPLENISH_PERIOD 15
#define TOTAL_CREDIT_IN_TIME_SLICE ( REPLENISH_PERIOD * ( MICRO_SEC(KTHREAD_VTALRM_SEC) + KTHREAD_VTALRM_USEC ) )
#define DEFAULT_UTHREAD_CREDIT 0 
#define MICRO_SEC(X) ( (X) * 1000000 )
...
typedef struct __kthread_runqueue
{
     ...
     unsigned int total_uthread_weight;  /*weight of all uthreads in this queue*/  
} kthread_runqueue_t;
...
inline void __rem_from_runqueue(runqueue_t *runq, uthread_struct_t *u_elem);
